...
if(!empty($_POST) && isset($_SESSION["user_id"])){
    ...
    $sql = 'insert into pr_insolvencias (iduser,nombreproceso,campo1) value ('.$_SESSION["user_id"].',"'.$nombreproceso.'","'.$campo1.'")';
...
//exit;
	...
	if ($query) {
		...
		header("location: ../newinsolvencia.php?success");
	} else {
		// echo "hubo un error al crear la carpeta";
		header("location: ../newinsolvencia.php?error");
	}
